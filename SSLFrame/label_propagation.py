...
def propagation(data, feature_data, alpha=0.75, unlabelled_ratio=0.3, iters=100):
    """
    标签传播
    :param data: 数据实例
    :param feature_data: 数据的特征向量
    :param alpha: 扩散率
    :param unlabelled_ratio: 未标记数据比例
    :param iters: CG求解迭代次数
    """
    ...
    for i in range(iters):
        ...
        pseudo_label, cg_solution, info = diffusion_learning(SSPTM, y_noisy, alpha=alpha, verbose=False)
        y_pred += pseudo_label
    end_time = time.time()
